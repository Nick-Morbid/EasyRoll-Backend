...
public class SocketContextHandler {
    ...
    public synchronized static void addContext(String key,SocketContext socketContext){
        log.warn("第三方授权长连接的socketId为：{}",key);
        contextPool.put(key,socketContext);
    }

    public synchronized static SocketContext getContext(String key){
    ...
    public synchronized static void clearContext(String key) throws IOException {
        if (contextPool.containsKey(key)){
